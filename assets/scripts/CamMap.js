...
cc.Class({
  ...
  startPositioningExistingStatefulBuildableInstance(statefulBuildableInstance) {
    ...
    if (null == statefulBuildableInstanceNode) {
      console.warn("There is no `statefulBuildableInstanceNode` when trying to position exiting statefulBuildableInstance.");
      return;
    }
  ...
  spawnOrRefreshStatefulBuildableFollowingNpcs() {
    ...
    for (let indice = 0; indice < self.statefulBuildableInstanceCompList.length; ++indice) {
      ...
      if (null != statefulBuildableInstanceComp.boundFollowingNpc) {
        console.warn("The `statefulBuildableInstanceComp` of uuid ", statefulBuildableInstanceComp.uuid, " already has a boundFollowingNpc of uuid ", statefulBuildableInstanceComp.boundFollowingNpc.uuid);
        continue;
      }
      ...
      statefulBuildableInstanceComp.boundFollowingNpc = npcScriptIns;
